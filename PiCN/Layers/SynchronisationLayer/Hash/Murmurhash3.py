...
class Murmurhash3(object):
    ...
    def hash(self, key, seed=0x0):
        ''' Implements 32bit murmur3 hash. '''

        key = bytearray(self.xencode(key))
        ...
        # body
        for block_start in self.xrange(0, nblocks * 4, 4):
            # ??? big endian?
    ...
    def hash128(self, key, seed=0x0, x64arch=True):
        ...
        def hash128_x64(key, seed):
            ...
            # body
            for block_start in self.xrange(0, nblocks * 8, 8):
                # ??? big endian?
        ...
        def hash128_x86(key, seed):
            ...
            # body
            for block_start in self.xrange(0, nblocks * 16, 16):
                k1 = key[block_start + 3] << 24 | \
                     key[block_start + 2] << 16 | \
                     key[block_start + 1] << 8 | \
                     key[block_start + 0]
    ...
    def hash64(self, key, seed=0x0, x64arch=True):
        ''' Implements 64bit murmur3 hash. Returns a tuple. '''

        hash_128 = self.hash128(key, seed, x64arch)
    ...
    def hash_bytes(self, key, seed=0x0, x64arch=True):
